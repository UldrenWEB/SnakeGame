...
window.addEventListener('load', ()=>{
    ...
    //Juego original
    let canva = document.querySelector("#miCanva");
    ...
    let canvaX = Math.abs(canva.width / 20).toFixed(0);//10 es el tamaño de pixel que queremos
    let canvaY = Math.abs(canva.height / 20).toFixed(0);
    ...
    function renderGrid() {
        ...
        for (let i = 0; i < canva.width; i += 20) {
            ctx.beginPath();
            ctx.moveTo(i, 0);
        ...
        for (let j = 0; j < canva.height; j += 20) {
            ctx.beginPath();
            ctx.moveTo(0, j);
